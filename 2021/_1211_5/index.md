...
GitHub サーフィンをしているとまれにコミットに署名（Verified）されたものがある。
これは一体どうなっているのかと調べたら GNU Privacy Guard という暗号化ソフトで作られた鍵を使用したものらしい。
...
まずは Homebrew で GPG をインストール。

```
brew install gpg
```

次に鍵を作成していく。なんか色々聞かれるけど答えていけば OK。
...
調べると Mac の場合、pinentry がないとパスフレーズを入れられないらしい。
Homebrew でインストールする。
...
公開鍵を GitHub に登録するためにクリップボードにコピー。設定からペーストする。
...
こんな感じにペーストされるので保存すれば完了。署名付きのコミットができていれば OK。
